...
def build_model(vocab, pretrained_decoder_fpath=None):
    decoder = Decoder(
    ...
    if pretrained_decoder_fpath is not None:
        decoder.load_state_dict(torch.load(pretrained_decoder_fpath)['decoder'])
        print("Pretrained decoder is loaded from {}".format(pretrained_decoder_fpath))
...
def log_train(summary_writer, e, loss, lr, scores=None):
    ...
    summary_writer.add_scalar(C.tx_train_pos_loss, loss['pos'], e)
    summary_writer.add_scalar(C.tx_train_neg_loss, loss['neg'], e)
    summary_writer.add_scalar(C.tx_lr, lr, e)
    print("loss: {} (P {} + N {})".format(loss['total'], loss['pos'], loss['neg']))
...
def log_val(summary_writer, e, loss, scores):
    ...
    summary_writer.add_scalar(C.tx_val_pos_loss, loss['pos'], e)
    summary_writer.add_scalar(C.tx_val_neg_loss, loss['neg'], e)
    for metric in C.metrics:
...
def main():
    ...
    target_model = build_model(vocab, C.target_decoder_fpath)
    reference_model = build_model(vocab, C.reference_decoder_fpath)

    optimizer = torch.optim.Adam(target_model.parameters(), lr=C.lr, weight_decay=C.weight_decay, amsgrad=True)
    ...
    for e in range(1, C.epochs + 1):
        ...
        """ Train """
        print("\n")
        train_loss = train(e, target_model, reference_model, optimizer, train_iter, vocab,
                           C.decoder.rnn_teacher_forcing_ratio, C.gradient_clip)
        log_train(summary_writer, e, train_loss, get_lr(optimizer))

        """ Validation """
        val_loss = test(target_model, reference_model, val_iter, vocab)
        val_scores = evaluate(val_iter, target_model, vocab, beam_width=5, beam_alpha=0.)
        ...
        if e >= C.save_from and e % C.save_every == 0:
            ...
            save_checkpoint(e, target_model, ckpt_fpath, C)
    ...
    print("\n\n\n[BEST]")
    best_target_model = load_checkpoint(target_model, best_ckpt_fpath)
    best_scores = evaluate(test_iter, best_target_model, vocab, beam_width=5, beam_alpha=0.)
    print("scores: {}".format(best_scores))
    for metric in C.metrics:
        summary_writer.add_scalar("BEST SCORE/{}".format(metric), best_scores[metric], best_epoch)
    save_checkpoint(e, best_target_model, C.ckpt_fpath_tpl.format("best"), C)
